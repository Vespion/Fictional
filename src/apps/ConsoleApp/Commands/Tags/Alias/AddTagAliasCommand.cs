...
internal abstract class AddTagAliasCommand: ICommand
{
	public static Command Configure()
	{
		var command = new Command("add", "Adds a tag alias")
		{
			CanonicalIdArg,
			AliasIdArg
		};
		...
		command.SetHandler(
			Handle,
			new LoggerBinder<AddTagAliasCommand>(),
	...
	internal static async Task<int> Handle(
		ILogger<AddTagAliasCommand> logger,
		ITagRepository repo,
		long canonicalTagId,
		long aliasTagId,
	...
	)
	{
		...
		var added = await repo.AddAliasLinkAsync((TagId)canonicalTagId, (TagId)aliasTagId, ctx);

		if (!added)
		{
			logger.LogWarning("Alias already exists");
