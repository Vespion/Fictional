...
internal record UInt64PrimitiveEmitter : DefaultSourceCodeEmitter
{
	...
	private static void WriteConversionOperators(SourceWriter writer, string typeName)
	{
		writer
			...
			.WriteLine($"public static explicit operator {typeName}(ulong value) => new(value);")
			.WriteLine($"public static implicit operator ulong({typeName} value) => value.Value;");
	}
	...
	private void TryFormatCharsBlock(SourceWriter writer)
	{
		writer
			.WriteInheritanceDocComment()
			.WriteLine(
				$"public bool TryFormat(Span<char> destination, out int charsWritten, {StringSyntaxAttribute} ReadOnlySpan<char> format, IFormatProvider? provider) => Value.TryFormat(destination, out charsWritten, format, provider);");
	}
	...
	private void TryFormatUtf8Block(SourceWriter writer)
	{
		writer
			.WriteInheritanceDocComment()
			.WriteLine(
				$"public bool TryFormat(Span<byte> utf8Destination, out int bytesWritten, {StringSyntaxAttribute} ReadOnlySpan<char> format, IFormatProvider? provider) => Value.TryFormat(utf8Destination, out bytesWritten, format, provider);");
	}
	...
	private void ParseSpanBlock(SourceWriter writer, string typeName)
	{
		...
		writer.OpenBlock();
		writer.WriteLine("var value = ulong.Parse(s, provider);");
	...
	private void TryParseSpanBlock(SourceWriter writer, string typeName)
	{
		...
		writer.OpenBlock();
		writer.WriteLine("if(!ulong.TryParse(s, provider, out var value))");
		writer.OpenBlock();
