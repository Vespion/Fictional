...
internal record Int64PrimitiveEmitter : DefaultSourceCodeEmitter
{
	...
	private static void WriteConversionOperators(SourceWriter writer, string typeName)
	{
		writer
			...
			.WriteLine($"public static explicit operator {typeName}(long value) => new(value);")
			.WriteLine($"public static implicit operator long({typeName} value) => value.Value;");
	}
	...
	private void ParseSpanBlock(SourceWriter writer, string typeName)
	{
		...
		writer.OpenBlock();
		writer.WriteLine("var value = long.Parse(s, provider);");
	...
	private void TryParseSpanBlock(SourceWriter writer, string typeName)
	{
		...
		writer.OpenBlock();
		writer.WriteLine("if(!long.TryParse(s, provider, out var value))");
		writer.OpenBlock();
